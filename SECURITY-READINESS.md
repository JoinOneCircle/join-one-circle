...
2. Run migrations `0001` through `0005` in a disposable Supabase project first. Review the SQL Editor result, function grants and RLS policies; then repeat in the client-owned project. A migration file existing in the repository does not mean it ran.
...
7. Add persistent rate limits and usage ceilings for Circle AI. In-memory per-instance throttling is not a production control. Test that AI input contains only the current user's permitted record areas and does not accept another child's conversation ID.
...
- `supabase/0005_security_and_live_workflows.sql` must be independently tested before deployment. In particular, verify pending document uploads against metadata RLS, prevent updates to document identity/status, prevent invitation acceptance from restoring administrative access, and ensure institutional verification status is enforced where roles affect data access.

...
export default async function ChildRecord({ params, searchParams }: { params: Promise<{ childId: string }>; searchParams: Promise<{ area?: string; error?: string; message?: string }> }) {
  ...
  return <>
    ...
    {activeArea && activeSection && <section className="panel record-editor" id="record-area">
      ...
      {activeItems.length ? <div className="record-item-list">{activeItems.map((item) => <article key={item.id}><div><h3>{item.title}</h3>{itemSummary(item.body) && <p>{itemSummary(item.body)}</p>}<small>Updated {new Intl.DateTimeFormat("en", { dateStyle: "medium" }).format(new Date(item.updated_at))}</small></div><form action={deleteRecordItem}><input type="hidden" name="child_id" value={childId} /><input type="hidden" name="item_id" value={item.id} /><input type="hidden" name="record_area" value={activeArea} /><button className="quiet-button" type="submit">Delete</button></form></article>)}</div> : <p className="empty-filter">Nothing has been added to this area yet.</p>}

...
export default async function ChildRecord({ params, searchParams }: { params: Promise<{ childId: string }>; searchParams: Promise<{ area?: string; error?: string; message?: string }> }) {
  ...
  const { child, items, access } = await getChildRecord(childId);
  const canReadArea = (area: string) => access.canManage || access.readAreas.includes(area) || access.contributeAreas.includes(area);
  const canContributeArea = (area: string) => access.canManage || access.contributeAreas.includes(area);
  const activeArea = sections.some(([, , area]) => area === query.area && canReadArea(area)) ? query.area! : null;
  ...
  const primarySections = sections.filter(([, , area]) => startingAreas.has(area) && canReadArea(area));
  const additionalSections = sections.filter(([, , area]) => !startingAreas.has(area) && canReadArea(area));
  ...
  return <>
    ...
    {activeArea && activeSection && <section className="panel record-editor" id="record-area">
      ...
      {activeItems.length ? <div className="record-item-list">{activeItems.map((item) => { const attachment = itemAttachment(item.body); return <article key={item.id}><div><h3>{item.title}</h3>{itemSummary(item.body) && <p data-no-translate>{itemSummary(item.body)}</p>}<small>Updated <LocalizedDate value={item.updated_at} /></small><RecordItemActions title={item.title} summary={itemSummary(item.body)} area={activeSection[0]} updatedAt={item.updated_at} attachment={attachment} /></div>{access.canManage && <ConfirmDeleteForm action={deleteRecordItem} values={{ child_id: childId, item_id: item.id, record_area: activeArea }} itemName={item.title} itemType="record update" triggerClassName="quiet-button" />}</article>; })}</div> : <p className="empty-filter">Nothing has been added to this area yet.</p>}
      {canContributeArea(activeArea) ? <div className="record-update-entry"><div><h3>Add an update</h3><p>Save what changed and, when useful, attach the supporting file directly to this update.</p></div><RecordUpdateForm action={createRecordItem} childId={childId} recordArea={activeArea} canManage={access.canManage} /></div> : <div className="record-update-entry record-update-entry--readonly"><div><h3>Read-only access</h3><p>You can read this part of the record, but you have not been given permission to add or change information here.</p></div></div>}
    </section>}
    {access.canManage && <section className="panel record-admin"><h2>Child details</h2><form action={updateChild} className="form-row"><input type="hidden" name="child_id" value={childId} /><label className="field">Preferred name<input name="child_name" defaultValue={child.preferred_name} maxLength={120} required /></label><label className="field">Date of birth<input name="date_of_birth" type="date" defaultValue={child.date_of_birth ?? ""} /></label><button className="quiet-button" type="submit">Save details</button></form><ConfirmDeleteForm action={deleteChild} values={{ child_id: childId }} itemName={`${child.preferred_name}'s child record`} itemType="child record" triggerLabel="Delete child record" /></section>}
  </>;
}

...
export function DemoChildRecord({ childId, area }: { childId: string; area?: string }) {
  ...
  const itemText = (item: typeof items[number]) => [`${activeSection[0]}: ${item.title}`, item.summary, item.occurred_on ? `Event date: ${item.occurred_on}` : ""].filter(Boolean).join("\n\n");
  const copyItem = async (item: typeof items[number]) => {
    try {
      await navigator.clipboard.writeText(itemText(item));
      setMessage("Update copied.");
    } catch { setError("The update could not be copied."); }
  };
  const downloadItemText = (item: typeof items[number]) => {
    const blob = new Blob([itemText(item)], { type: "text/plain;charset=utf-8" });
    const url = URL.createObjectURL(blob); const link = document.createElement("a");
    link.href = url; link.download = `${item.title.toLowerCase().replace(/[^a-z0-9]+/gi, "-").replace(/(^-|-$)/g, "") || "record-update"}.txt`;
    link.click(); window.setTimeout(() => URL.revokeObjectURL(url), 30_000);
  };
  ...
  return <>
    ...
    <section className="panel record-editor"><div className="panel-title"><div><p className="eyebrow">{activeSection[2].toUpperCase()}</p><h2>{activeSection[0]} updates</h2><p>Everything saved here is connected to this child&apos;s authorised record.</p></div></div><div className="record-item-list">{items.length ? items.map((item) => <article key={item.id}><div><h3>{item.title}</h3>{item.summary && <p data-no-translate>{item.summary}</p>}{item.occurred_on && <small>Event date: {item.occurred_on}</small>}{item.file_id && item.file_name && <div className="record-attachment"><AppIcon name="documents" size={16} /><span data-no-translate>{item.file_name}</span><button type="button" onClick={() => downloadAttachment(item.file_id!, item.file_name!)}><AppIcon name="download" size={15} />Download</button></div>}<div className="record-item-actions"><button type="button" onClick={() => copyItem(item)} title="Copy update"><AppIcon name="reports" size={15} />Copy</button><button type="button" onClick={() => downloadItemText(item)} title="Download update"><AppIcon name="download" size={15} />Download</button></div></div><button className="quiet-button" type="button" onClick={() => removeItem(item.id, item.file_id)}>Delete</button></article>) : <p className="empty-filter">No updates have been saved in this area yet.</p>}</div></section>

...
type VisibleCircleMember = {
  user_id: string;
  child_id: string;
  role: string;
  status: string;
  is_access_admin: boolean;
  permissions: { read_areas?: unknown } | null;
...
export default async function MyCirclePage() {
  ...
  if (isSupabaseConfigured) {
    ...
    // Do not derive this list from the visible child profile. A contributor
    // can have a deliberately limited grant and still needs to see their own
    // accepted access here; an access administrator can see the full circle.
    const { data } = await supabase!.rpc("list_visible_circle_members");
    people = ((data ?? []) as VisibleCircleMember[]).map((member) => ({ userId: member.user_id, childId: member.child_id, role: member.role, status: member.status, isAccessAdmin: member.is_access_admin, readAreas: Array.isArray(member.permissions?.read_areas) ? member.permissions.read_areas as string[] : [], name: member.display_name ?? "Authorised person" }));
  }

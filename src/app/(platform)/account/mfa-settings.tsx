...
export function MfaSettings() {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      void client.auth.mfa.listFactors().then(({ data }) => setEnabled(Boolean(data?.totp.some((factor) => factor.status === "verified")))).catch(() => setEnabled(false));
    }, 0);

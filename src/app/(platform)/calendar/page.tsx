...
import { cancelChildEvent, createChildEvent, respondToChildEvent } from "./actions";
...
export default async function CalendarPage({ searchParams }: { searchParams: Promise<{ error?: string; message?: string }> }) {
  ...
  return <>
    ...
    {childOptions.length > 0 && <section className="panel calendar-create"><div><p className="eyebrow">NEW EVENT</p><h2>Schedule something for a child</h2><p>The child&apos;s access administrators are invited automatically. Other participants can be added in the next iteration of the circle workflow.</p></div><form action={createChildEvent} className="form-row"><label className="field">Child<select name="child_id" required defaultValue=""><option value="" disabled>Choose a child</option>{childOptions.map((child) => <option key={child.id} value={child.id}>{child.preferred_name}</option>)}</select></label><label className="field">Title<input name="title" maxLength={200} required placeholder="e.g. Annual review" /></label><label className="field">Starts<input name="starts_at" type="datetime-local" required /></label><label className="field">Ends <span>(optional)</span><input name="ends_at" type="datetime-local" /></label><label className="field calendar-description">Details<textarea name="description" maxLength={2000} rows={3} placeholder="What should participants know?" /></label><button className="button button--small" type="submit">Create event</button></form></section>}

...
export default async function InvitationPage({ params, searchParams }: { params: Promise<{ token: string }>; searchParams: Promise<{ error?: string }> }) {
  ...
  const errorMessage = error === "invitation-not-available" ? "This invitation is not available. It may have expired, been used already or belong to a different email address." : error;
